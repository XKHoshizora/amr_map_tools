...
```cpp
ros::Publisher pub = nh.advertise<amr_map_tools::Waypoint>("/waypoint/add_waypoint", 1);
...
- 可通过 launch 文件的参数设置:
  ```xml
  <param name="waypoint_file" value="$(find amr_map_tools)/waypoints/default.xml"/>
  ```
